...
# 데이터베이스 테이블 초기화 (messages2로 강제 리셋하여 500 에러 완벽 해결)
def init_db():
    ...
    # 4. 채팅 메시지 테이블 (messages2로 격상하여 컬럼 충돌 전면 방지)
    cur.execute('''
...
# --- 🚀messages2 테이블과 100% 연동된 실시간 채팅 라우터 ---
...
def get_messages(room_type, room_id):
    conn = get_db_connection()
    cur = conn.cursor()
    # messages2 테이블에서 안전하게 데이터 추출
    cur.execute('SELECT sender, message, image_url FROM messages2 WHERE room_type = %s AND room_id = %s ORDER BY id ASC', (room_type, room_id))
    rows = cur.fetchall()
    cur.close()
    conn.close()
...
def send_message():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    # messages2 새 테이블 금고에 완벽 저장
    cur.execute('INSERT INTO messages2 (room_type, room_id, sender, message, image_url) VALUES (%s, %s, %s, %s, %s)',
